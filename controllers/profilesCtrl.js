...
const profilesCtrl = {
    ...
    updateMedicalProfile: async (req, res) => {
        try {
            const {name, bloodGroup, age, speciality_name, city_name, clinic_address, experience_year} = req.body.profile;
            await MedicalProfile.findOneAndUpdate(
                { userId: req.user.id },
                {name, bloodGroup, age, speciality_name, city_name, clinic_address, experience_year}
            );
            res.json({ msg: "Profile updated!" });
        } catch (err) {
        return res.status(500).json({ msg: err.message });
